...
layout = [
    ...
    [
        sg.Column(left_half),
        sg.VSeparator(),
        sg.Column(right_half)
    ]
]

#  create the window
window = sg.Window("Weather Forecast", layout,
                   margins=(10, 10), size=(860, 110))
...
while True:
    ...
    if event == "Show":
        ...
        try:
            temperature = "The current temperature in " + \
                data['timezone'] + " is " + str(data['current']['temp']) + \
                " degrees Celsius."

            temperature_tomorrow = "The temperature tomorrow will be " + \
                str(data['daily'][1]['temp']['day']) + " during the day and " + \
                str(data['daily'][1]['temp']['night']) + " during the night."
